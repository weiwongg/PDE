function [u, A, M, b, fns] = fem_for_heat_2d_triangle(c4n,n4e,n4db, ...
    ind4e,M_R,Srr_R,Srs_R,Ssr_R,Sss_R,f,u_D, time_step, pre_u)
%fem_for_heat_2d_triangle    Forward Euler method and FEM solver for heat equation problem in 2D with triangular elements
%   Parameters:
...
%     - pre_u: u at the previous time step
%
%   Returns:
...
u(n4db) = u_D(c4n(n4db,:));
u(fns) = M(fns,fns)\((M(fns, fns) - time_step * A(fns, fns)) * pre_u(fns) +time_step * b(fns));
end
